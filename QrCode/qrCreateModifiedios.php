...
$sqlKidCount="SELECT adults,kids,theme_park_id FROM `order` where id ='$order'";
...
if(!$AllTickets)
{
  echo json_encode(array('status'=>404,'message'=>"Please Contact Us For Access To This App"),JSON_PRETTY_PRINT);  exit;
	...
	$result = mysqli_query($db, $sql);
	 ...
	 echo json_encode(array('status'=>200,'message'=>'You have been successfully logged in','adultCount'=>"$TotalAdultCount",'kidCount'=>"$TotalKidCount",'ThemeParkParenId'=>"$ThemeParkParentId",'ThemeParkName'=>"$ThemeParkName",'orderDetails'=>$AllTickets),JSON_PRETTY_PRINT);
}
  /*  
}

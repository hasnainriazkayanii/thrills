...
if(!$AllTickets)
{
  echo json_encode(array('status'=>404,'message'=>"Please Contact Us For Access To This App"),JSON_PRETTY_PRINT);  exit;
	...
	$result = mysqli_query($db, $sql);
	 ...
	 echo json_encode(array('status'=>200,'message'=>'Logged In! Click OK To Continue.','adultCount'=>"$TotalAdultCount",'kidCount'=>"$TotalKidCount",'ThemeParkParenId'=>"$ThemeParkParentId",'ThemeParkName'=>"$ThemeParkName",'orderDetails'=>$AllTickets,'dateOfvisit'=>$dov,'screenshot'=>$is_screenchot),JSON_PRETTY_PRINT);
}
  /*  
}

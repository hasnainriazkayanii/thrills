...
if (isset($_POST['adminsubmit'])) {
  ...
  $access_level = $_POST['level'] ?? 0;

  $admin_insert = "INSERT INTO login_user (user_name, email, password, mob_no, status, level)
        VALUES ('$name','$email','$password','$mobnumber','$type', '$access_level')";

  $result = mysqli_query($db, $admin_insert);

  $last_id = mysqli_insert_id($db);

  $partner_insert = "INSERT INTO partners (name, setComission, loginID, active)
        VALUES ('$name','$comission','$last_id', '1')";
      ...
      <div class="col-md-7">
        ...
        <form action="AddRole.php" autocomplete='off' method="post">
          ...
          <div class="form-group">
            ...
            <select class="form-control" name="type" onchange="role(this);">
            ...
            <div id="level">
              ...
              <select class="form-control" name="level">
                ...
                <option value="2">Sales Manager - 2</option>
            ...
            <div class="form-group">
              <label for="mobnumber">Commision *</label>
              <input type="text" maxlength="10" onkeypress="return AllowNumbersOnly(event)" required class="form-control" name="comission" id="comission" aria-describedby="comission" placeholder="comission *" value="">
              <span id="comission" class="text-danger font-weight-bold"></span>
            </div>
...
<script type="text/javascript">
  ...
  function role(element) {
    console.log(element);
    // Takes the element of role select
    if (element.value == 0) { //check if its user or admin
      $('#level').hide();
    } else {
      $('#level').show();
    }

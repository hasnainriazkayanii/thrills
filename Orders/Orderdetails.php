...
//  var_dump($sql);
        ...
        $sql = "SELECT *,(SELECT SUM(paymentAmount) from accounting where orderID = `order`.order_id) as amount_sum,order.customer_id as order_customer_id,g.id as guest_id, order.id as order_id, order.order_id as order_code, theme_parks.id as theme_park_id, theme_parks.name AS park_name, customer.Phone_number, order.customer_id as customer_code, accounting.orderID, SUM(accounting.paymentAmount) as sum_amount FROM `order` INNER JOIN `theme_parks` ON order.theme_park_id=theme_parks.id INNER JOIN `customer` ON order.customer_id=customer.id LEFT JOIN `accounting` ON order.order_id=accounting.orderID Left JOIN guest as g ON order.id= g.order_id LEFT JOIN partners pa ON pa.id = order.sales_personID WHERE order.date_of_visit>=$current_date $last_3_days_check $ishide AND order.status  not in (3) group by order.order_id ORDER BY order.date_of_visit ASC,timestamp(order.time) ASC,STR_TO_DATE(time, '%l:%i %p')";
    }
    else{

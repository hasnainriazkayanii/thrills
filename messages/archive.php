...
if (! isset($_REQUEST['all']))
{
   ...
   {
      $sql = "SELECT count(id) as count FROM `customer` WHERE Phone_number IN (SELECT contact_no FROM `archived_chats`)";
      ...
      $sql = "SELECT * FROM `customer` WHERE Phone_number IN (SELECT contact_no FROM `archived_chats`) ORDER BY (SELECT created_at FROM `massages` WHERE contact_no=customer.Phone_number AND seen=0 AND type='recieved' ORDER BY created_at DESC LIMIT 1) DESC LIMIT $offset, $limit";
      $result = mysqli_query($db, $sql);
   ...
   else
   {
      ...
      $sql = "SELECT count(id) as count FROM `customer` WHERE Phone_number IN (SELECT contact_no FROM `archived_chats`) AND first_name like '%".$search_query."%' OR Last_name like'%".$search_query."%' OR Phone_number like'%".$search_query[0]."%' OR homecity like'%".$search_query."%' ORDER BY id  DESC";
      $result = mysqli_query($db, $sql);
      $total_records = (int) mysqli_fetch_assoc($result)['count'];
      ...
      $sql = "SELECT * FROM `customer` WHERE Phone_number IN (SELECT contact_no FROM `archived_chats`) AND CONCAT(first_name,' ', Last_name) LIKE '%$search_query%' OR first_name like '%".$search_query."%' OR Last_name like'%".$search_query."%' OR Phone_number like'%".$search_query[0]."%' OR homecity like'%".$search_query."%' ORDER BY id  DESC LIMIT $offset, $limit";
      $result = mysqli_query($db, $sql);
      $members = mysqli_fetch_all($result, MYSQLI_ASSOC);
   }
...
else
{
   if (! isset($_REQUEST['search_query']))
   {
      $sql = "SELECT * FROM `customer` WHERE Phone_number IN (SELECT contact_no FROM `archived_chats`) ORDER BY (SELECT created_at FROM `massages` WHERE contact_no=customer.Phone_number AND seen=0 AND type='recieved' ORDER BY created_at DESC LIMIT 1) DESC";
      $result = mysqli_query($db, $sql);
      $members = mysqli_fetch_all($result, MYSQLI_ASSOC);
   }
   else
   {
      ...
      $sql = "SELECT * FROM `customer` WHERE Phone_number IN (SELECT contact_no FROM `archived_chats`) AND CONCAT(first_name,' ', Last_name) LIKE '%$search_query%' OR first_name like '%".$search_query."%' OR Last_name like'%".$search_query."%' OR Phone_number like'%".$search_query[0]."%' OR homecity like'%".$search_query."%' ORDER BY id  DESC";
      $result = mysqli_query($db, $sql);
      $members = mysqli_fetch_all($result, MYSQLI_ASSOC);
   }
}
...
{
   // Get last message
   $sql = "SELECT * FROM `massages` WHERE contact_no='{$mbr['Phone_number']}' ORDER BY created_at DESC";
   $result = mysqli_query($db, $sql);
   $last_message = mysqli_fetch_assoc($result);
   $mbr['last_message'] = $last_message;
		...
		<div class="row">
    		<div class="col-md-12">
    			<div class="col-md-8 new-header" style="float:left;">
    				<h3 class="new-fonts">Archive</h3>
    			</div>
    			<div class="col-md-4 text-right new-header" style="float:left;" >
               <a href="index.php" class="btn btn-warning">Back to Messages</a>
                              ...
                              <table class="table table-bordered dataTable no-footer" id="dataTable" role="grid" aria-describedby="dataTable_info" style="width: 100%;" width="100%" cellspacing="0">
                                 <thead>
                                    <tr id="th-new" role="row">
                                       ...
                                       <th class="th-1 sorting_asc" tabindex="0" aria-controls="dataTable" rowspan="1" colspan="1" style="width: 217px;" aria-sort="ascending" aria-label="Name: activate to sort column descending">Last Message</th>
                                       ...
                                       <th class="th-7 sorting" tabindex="0" aria-controls="dataTable" rowspan="1" colspan="1" style="width: 185px;" aria-label="Home City: activate to sort column ascending">Home City</th>
                                       <th class="th-7 sorting" tabindex="0" aria-controls="dataTable" rowspan="1" colspan="1" style="width: 185px;" aria-label="Home City: activate to sort column ascending">Subscribed</th>
                                 ...
                                 <tbody>
                                    <?php foreach ($members as $member) { ?>
                                       <tr role="row" class="odd <?php if($member['last_message']['seen'] === '0') echo 'font-weight-bold'; ?>">
                                          <td class="sorting_1">
                                          ...
                                          <td>
                                             <?php if ($member['last_message']): ?>
                                                <?php echo strlen($member['last_message']['message']) > 40? substr($member['last_message']['message'], 0, 40)."...": $member['last_message']['message']; ?>
                                             <?php else: ?>
                                                No messages yet
                                             <?php endif; ?>
                                          </td>
                                          ...
                                          <td>
                                             <a href="remove_from_archive.php?contact_no=<?php echo $member['Phone_number']; ?>" class="btn btn-primary">Remove From Archive</a>

            ...
            <div class="col-md-12">
                ...
                <div class="col-md-4 text-right new-header" style="float:left;">
                    ...
                    <button type="button" class="btn btn-primary" data-toggle="modal"
                        data-target="#add-text-message-modal">Add New</button>
                </div>
            </div>
        </div>
        <hr>
                ...
                <table class="table table-hover w-100 border">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="message"><?php echo substr($message['message'],0,80); ?><input type="hidden"
                                    class="message_hidden" value="<?php echo $message['message']; ?>" />
                                    <input type="hidden"
                                    class="admin_message_hidden" value="<?php echo $message['admin_message']; ?>" /></td>
                            <td>
                                <button type="button" class="btn btn-warning edit-text-message-btn" data-toggle="modal"
                                    data-id="<?php echo $message['id'] ?>" data-target="#edit-text-message-modal"
                                    data-image="<?php echo $message['message_attachment'] ?>"
                                    data-status="<?php echo $message['status'] ?>"
                                    data-sendto='<?=$message['send_to']?>'
                                    data-park='<?php echo @$message['theme_park_id'] ?>'>Edit</button>
                                <a href="delete_text_message.php?id=<?php echo $message['id']; ?>"
            ...
            <form action="create_text_message.php" method="post" enctype="multipart/form-data">
                ...
                ?>
                  ...
                  ?>
                    ...
                    <div class="form-group">
                        <label class="font-weight-bold">Admin Message</label>
                        <textarea name="admin_message"  class="form-control" rows="10"></textarea>
                    </div>
                ...
                while($row=mysqli_fetch_assoc($res)){ ?>
                        <div class="form-check form-check-inline">
                            <input class="form-check-input" name="send_to[]" type="checkbox" id="<?=$row['id']?>" value="<?=$row['id']?>">
            ...
            <form id="edit-text-message-form" action="" method="post" enctype="multipart/form-data">
                ...
                ?>
                  ...
                  ?>
                    ...
                    <div class="form-group">
                        <label class="font-weight-bold">Admin Message</label>
                        <textarea name="admin_message" id="admin-message-edit-field" class="form-control" rows="10"></textarea>
                    </div>
                    <div class="form-group">
                        <label class="font-weight-bold">Send To</label>
                ...
                while($row=mysqli_fetch_assoc($res)){ ?>
                        <div class="form-check form-check-inline">
                            <input class="form-check-input" name="send_to[]" type="checkbox" id="e-<?=$row['id']?>" value="<?=$row['id']?>">
                            <label class="form-check-label" for="e-<?=$row['id']?>"><?=$row['user_name']?></label>
                        </div>
                        <?php }
                ?>
                    ...
                    <div class="form-group">
                        ...
                        <input type="file" class="form-control mt-2" name="edit-message-attach"
                            id="edit-message-attach">
...
$('.edit-text-message-btn').click(function() {
  $('input[name="send_to[]"]').prop('checked', false);
    ...
    var admin_message = $(this).closest('tr').find('.admin_message_hidden').val();
    ...
    if(sendTo.length>0){
      sendTo.forEach(function(element) {
    ...
    $('#admin-message-edit-field').val(admin_message);
